...
class LogClustering(object):
    ...
    def compute_min_inter_cluster_dist(self):
        min_inter_cluster_dist = float('inf')
        for i in range(len(self.representatives)):
            for j in range(i + 1, len(self.representatives)):
                dist = self._distance_metric(self.representatives[i], self.representatives[j])
                if dist < min_inter_cluster_dist:
                    min_inter_cluster_dist = dist
        return min_inter_cluster_dist

    def compute_avg_inter_cluster_dist(self):
        total_inter_cluster_dist = 0
        num_of_dists = 0
        for i in range(len(self.representatives)):
            for j in range(i + 1, len(self.representatives)):
                dist = self._distance_metric(self.representatives[i], self.representatives[j])
                ...
                num_of_dists += 1
        return total_inter_cluster_dist / num_of_dists

    def compute_min_intra_cluster_dist(self):
        min_intra_cluster_dist = float('inf')
        for cluster_id, size in self.cluster_size_dict.items():
            if size > 1:
                cluster_members = np.array([x for x, y in zip(self.X, self.cluster_index) if y == cluster_id + 1])
                if not cluster_members.any():
                    continue
                intra_cluster_dists = pdist(cluster_members, metric=self._distance_metric)
                min_intra_cluster_dist_in_current_cluster = np.min(intra_cluster_dists)
                if min_intra_cluster_dist_in_current_cluster < min_intra_cluster_dist:
                    min_intra_cluster_dist = min_intra_cluster_dist_in_current_cluster
        return min_intra_cluster_dist

    def compute_max_intra_cluster_dist(self):
        ...
        for cluster_id, size in self.cluster_size_dict.items():
            if size > 1:
                ...
                if not cluster_members.any():
                    continue
                intra_cluster_dists = pdist(cluster_members, metric=self._distance_metric)
                max_intra_cluster_dist_in_current_cluster = np.max(intra_cluster_dists)
                if max_intra_cluster_dist_in_current_cluster > max_intra_cluster_dist:
                    max_intra_cluster_dist = max_intra_cluster_dist_in_current_cluster
