...
class FeatureExtractor(BaseEstimator):
    ...
    def fit(self, session_dict):
        ...
        self.id2log_train = {0: log_padding, 1: log_oov,2:"<pad1>",3:"<pad2>",4:"<pad5>"}
    ...
    def transform(self, session_dict, datatype="train"):
        ...
        if datatype == "test":
            print(f'before test num of templates: {len(self.id2log_train)}')
            # handle new logs
            ulog_new = ulog - set(self.ulog_train)
            logging.info(f"{len(ulog_new)} new templates show while testing.")
            self.ulog_train.extend(list(ulog_new))
            self.id2log_train.update({idx: log for idx, log in enumerate(self.ulog_train, 5)})
            # ulog_new = sorted(list(ulog_new))
            # for newlog in ulog_new:
            #     self.id2log_train.update(
            #         {idx: log for idx, log in enumerate(ulog_new, len(self.ulog_train)+5)}
            #     )
            #     self.ulog_train.append(newlog)
            # log2id_train：各种日志模板以及对应的编号（从0开始）,包括预留的字符
            self.log2id_train = {v: k for k, v in self.id2log_train.items()}
            print(f'after test num of templates: {len(self.id2log_train)}')
        ...
        for session_id, data_dict in session_dict.items():
            ...
            if self.feature_type == "sequentials" or self.feature_type=="stransformer":
